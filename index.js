...
const ALLOWED_ROLE_ID = '1506121132854611989';
...
client.once('ready', async () => {
    ...
    const updateChannel = client.channels.cache.find(
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.commandName === 'unraidmode') {

        raidMode = false;

        return interaction.reply({
            content: '✅ Raid mode disabled.',
            ephemeral: true
        });
    }
    ...
    if (interaction.commandName === 'botinfo') {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                {
                    name: 'Raid Mode',
                    value: raidMode ? 'Enabled' : 'Disabled'
                }
            )
            .setTimestamp();
...
client.on('guildMemberAdd', async (member) => {
    ...
    joins = joins.filter(
        time =>
            Date.now() - time < JOIN_TIME
    );

    if (
        joins.length >= JOIN_LIMIT &&
        !raidMode
    ) {
